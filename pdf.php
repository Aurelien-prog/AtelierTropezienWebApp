...
if ($data['marque'] == "ATELIER TROPEZIEN") {
  ...
  class PDF extends FPDF {
    ...
    function Header() {
      // Logo : 1 >position à gauche du document (en mm), 2 >position en haut du document, 80 >largeur de l'image en mm). La hauteur est calculée automatiquement.
      $this->Image('PHOTO/TAMPON-ANA.png',30,0,150);//Seulement des images en png !
      // Saut de ligne 20 mm
      $this->Ln(5);
      // Titre gras (B) police Helbetica de 11
      $this->SetFont('Helvetica','B',25);
...
// Cellule avec les données du sous-titre sur 2 lignes, pas de bordure
$pdf->Cell(25,10,"Reference :   ".$data['reference']."",0,1,'L');
$pdf->Cell(25,10,"Couleur :       ".$data['couleur']."",0,1,'L');
$pdf->Image('PHOTO/'.$data['addrsImage'].'',120,50,60);
$pdf->Ln(5);
$pdf->Cell(25,10,"Pack de :       ".$data['colisage']."",0,1,'L');
$pdf->Ln(15);
$pdf->SetFillColor(255,255,255);
$pdf->Image('PHOTO/'.$data['addrsScancode'].'',120,50,60);
//$pdf->Cell(90,10,"CODE EAN : ".$data['CodeEAN']."",0,0,1,'C');
//$pdf->Image('PHOTO/'.$data['addrsScancode'].'',20,100,70);
